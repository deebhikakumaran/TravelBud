...
from main import food_agent as langgraph_food_agent
import time
import requests
...
async def handle_message(ctx: Context, sender: str, msg: ChatMessage):
    try:
        ...
        error_msg = ChatMessage(
            content=[TextContent(text=f"Food search error: {str(e)}")],
            msg_id=str(uuid4()),
            timestamp=datetime.now()
        )
...
def food_agent_wrapper(query) -> str:  
    try:
        ...
        result = langgraph_food_agent.invoke({"messages": [{"role": "user", "content": structured_input}]})
...
def extract_langgraph_content(result):
    """Extract content from LangGraph food agent result"""
    try:
...
food_agent.include(chat_proto, publish_manifest=True)

# Register with Agentverse
tool = LangchainRegisterTool()
agent_info = tool.invoke({
    "agent_obj": food_agent_wrapper,
    "name": "ParadoxFoodAgent",
    "port": 8003,  # Must match agent's port
    "description": "Food search agent",
